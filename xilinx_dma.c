...
static void xilinx_dma_chan_desc_cleanup(struct xilinx_dma_chan *chan)
{
	...
	spin_lock_irqsave(&chan->lock, flags);
	printk(KERN_INFO "in xilinx_dma_chan_desc_cleanup, after spin_lock_irqsave \n");

	list_for_each_entry_safe(desc, next, &chan->done_list, node) {
		struct dmaengine_result result;

		printk(KERN_INFO "in xilinx_dma_chan_desc_cleanup, in list_for_each_entry_safe \n");
		if (desc->cyclic) {
			xilinx_dma_chan_handle_cyclic(chan, desc, &flags);
			printk(KERN_INFO "in xilinx_dma_chan_desc_cleanup, after xilinx_dma_chan_handle_cyclic \n");
			break;
		}
		...
		printk(KERN_INFO "in xilinx_dma_chan_desc_cleanup, after dmaengine_desc_get_callback_invoke \n");
		spin_lock_irqsave(&chan->lock, flags);
...
static void xilinx_dma_do_tasklet(unsigned long data)
{
	...
	printk(KERN_INFO "in xilinx_dma_do_tasklet \n");
...
static irqreturn_t xilinx_dma_irq_handler(int irq, void *data)
{
	...
	printk(KERN_INFO "in xilinx_dma_irq_handler, read XILINX_DMA_REG_DMASR: %u \n", status);
	...
	printk(KERN_INFO "in xilinx_dma_irq_handler, before tasklet_schedule \n");
				  ...
				  struct device_node *node, int chan_id)
{
	...
	printk(KERN_INFO "xilinx_dma.c chan->irq: %d\n", chan->irq);
				    ...
				    struct device_node *node)
{
	...
	printk(KERN_INFO "in xilinx_dma_child_probe\n");
...
static int xilinx_dma_probe(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "in xilinx_dma_probe\n");
