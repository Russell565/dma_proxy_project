...
#define BUFFER_SIZE ((31 * 1024 * 1024) - 2)
...
static void sync_callback(void *completion)
{
	 ...
	 */
	complete(completion);
	printk(KERN_INFO "sync_callback(void *completion) ok\n");
}
...
static void wait_for_transfer(struct dma_proxy_channel *pchannel_p)
{
	...
	printk(KERN_INFO "in wait_for_transfer\n");
	...
	printk(KERN_INFO "after dma_async_is_tx_complete\n");
	if (timeout == 0)  {
...
static int create_channel(struct platform_device *pdev, struct dma_proxy_channel *pchannel_p, char *name, u32 direction)
{
	...
	printk(KERN_INFO "Size of pchannel_p->buffer_table_p: %zu\n", sizeof(struct channel_buffer) * BUFFER_COUNT);
...
static irqreturn_t dma_proxy_rx_irq_handler(int irq, void *data)
{
    struct dma_proxy_channel *pchannel = data;
    static int current_buffer = 0;
	int bdindex = pchannel->bdindex;

	printk(KERN_INFO "in dma_proxy_rx_irq_handler\n");
    /* This function is called when the DMA interrupt occurs, the DMA
     * transfer is complete at this point.
     */

    // /* Read the data from the current DMA buffer. */
    // memcpy_fromio(pchannel->buffer_table_p[current_buffer].buffer, pchannel->bdtable[bdindex].dma_handle + (current_buffer * sizeof(struct channel_buffer) * BUFFER_COUNT), sizeof(struct channel_buffer) * BUFFER_COUNT);

    // /* Switch to the other DMA buffer. */
    // current_buffer = (current_buffer + 1) % 2;

    // /* Notify the user space application that the data is available. */
    // complete(&pchannel->bdtable[bdindex].cmp);
...
static int dma_proxy_probe(struct platform_device *pdev)
{
    ...
    * from the device tree
	...
	printk(KERN_INFO "befor platform_get_resource\n");
	res = platform_get_resource(pdev, IORESOURCE_IRQ, 0);
	printk(KERN_INFO "after platform_get_resource\n");
	irq = res->start;
	printk(KERN_INFO "Request the interrupt: %d\n", irq);
	/* Request the interrupt */
	err = request_irq(irq, dma_proxy_rx_irq_handler, IRQF_SHARED,
			  "dma_proxy_rx_irq", pdev);
	if (err) {
